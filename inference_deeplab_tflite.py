# /**
#  * @file inference_deeplab_tflite.py 
#  * @author Samay Pashine
#  * @updated Samay Pashine
#  * @brief Code to track the ball in the frames on video using tflite model. 
#  * @version 3.5
#  * @date 2023-10-26
...
if __name__ == '__main__':
    ...
    log_path = r'logs/inference-deeplab-tflite' + os.sep + datetime.now().strftime('%Y-%m-%d-%H-%M-%S') + '.log'
    os.makedirs(r'logs/inference-deeplab-tflite', exist_ok=True)
    ...
    parser = argparse.ArgumentParser(description='Script to infer on the data using the deeplab ball tracking model.')
    ...
    for video_file in os.listdir(DATA_DIR):
        ...
        if SAVE_FLAG:
            ...
            os.makedirs(rf'{os.path.join("DeepLab Inference TFLITE Videos")}', exist_ok=True)
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            out = cv2.VideoWriter(os.path.join("./DeepLab Inference TFLITE Videos", video_file), fourcc, FRAME_RATE, IMAGE_SIZE)
        ...
        while True:
            ...
            output[prediction[0] == 1] = color_dict[1]
            output[prediction[0] == 2] = color_dict[2]
            stump_output[prediction[0] == 2] = color_dict[2]
